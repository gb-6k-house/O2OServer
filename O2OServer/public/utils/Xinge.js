...
var API_CREATE_MUTILPUSH           = 'http://openapi.xg.qq.com/v2/push/create_multipush';
...
function XingeApp(accessId, secretKey){
	...
	/**
	 * 推送消息给批量设备
	 * @param {array}    devices    账号数组
	 * @param {Message}  message     推送的消息
	 * @param {int}      environment 向iOS设备推送时必填，1表示推送生产环境；2表示推送开发环境。Android可不填
	 * @param {Function} callback    回调函数
	 */
	this.pushByDevices = function(devices, message, environment, callback){

		//效验devices
		if(!util.isArray(devices) || devices.length === 0){
			throw new Error('devices is invalid');
		}else{
		...
		//调用API
		callAPI(API_CREATE_MUTILPUSH, params, "POST", 10000, this.secretKey, function(e, ret){
			if(e){
				throw  new Erro(e);
			}else {
				//构造请求参数
